...
export function PlayButton({ playing = false, loading = false, onClick, size = 'md', label }: { playing?: boolean; loading?: boolean; onClick?: () => void; size?: 'sm' | 'md' | 'lg'; label?: string }) {
  ...
  return (
    <button type="button" onClick={onClick} aria-label={label || (playing ? 'Pause' : 'Play')} className={cn('flex shrink-0 items-center justify-center rounded-full bg-gold text-black transition hover:scale-105 hover:bg-gold-bright focus:outline-none focus:ring-2 focus:ring-gold focus:ring-offset-2 focus:ring-offset-ink', sizes[size])}>
